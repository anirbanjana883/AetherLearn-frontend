...
function AllCourses() {
  ...
  return (
    <div className="flex min-h-screen bg-gray-50">
      <Nav />
      <button className="fixed top-20 left-4 z-50 bg-white text-black px-3 py-1 rounded md:hidden
       border-2 border-black"
        onClick={()=>setVisibleSideBar(prev => !prev)}
       >
        {visibleSideBar ? 'Hide' : 'Show'} Filters
      </button>

      {/* filter sectioomn */}
      <aside
        className={`w-[260px] h-screen overflow-y-auto bg-black fixed top-0 left-0 p-6 py-[130px] 
         border-r border-gray-200 shadow-md transition-transform duration-300 z-10
         ${visibleSideBar ? "translate-x-0" : "-translate-x-full"} md:block md:translate-x-0`}
      >
        ...
        >
          <button
            className="px-[10px] py-[10px] bg-black text-white rounded-[10px] text-[15px] font-light
                     flex items-center justify-center gap-2 cursor-pointer"
                     onClick={()=>navigate("/search")}
          >
            Search With AI{" "}
            <img src={aiimg} className="w-[30px] h-[30px] rounded-full" />
          </button>
      ...
      <main
        className="w-full transition-all duration-300 py-[130px] md:pl-[300px] flex items-start
